...
from .user_intent_delegation import authenticated_user_intent_authority
...
class SmartAgentConversation(
    ...
    async def async_process(
        ...
        if not is_current_human_user(user):
            reply_text = "当前语音会话未绑定有效用户，未执行设备操作。"
        ...
        # deliberately not used to mint execution authority.
        user_intent_authority = None
        try:
            user_intent_authority = authenticated_user_intent_authority(
                user_id=user_id,
                session_id=str(getattr(context, "id", "") or ""),
            )
        except ValueError:
            _LOGGER.warning(
                "[Conversation API] authenticated HA context has no usable session; "
                "voice execution delegation remains unavailable"
            )
